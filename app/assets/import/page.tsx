...
const COMMON_COLUMN_NAMES = [
  ...
  'warranty_expiry', 'warrantyExpiry', 'warranty',
  ...
  return (
    <div>
      <PageHero
        ...
        description="Import multiple assets from CSV file"
      />
          ...
          <div className="card-body">
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Drag and drop or click to select a CSV file containing asset data
              </p>
              <input
                type="file"
                accept=".csv"
        ...
        <div className="card">
          <div className="card-header">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Specify which columns contain asset data. Auto-detected columns are highlighted.
            </p>
          </div>
          <div className="card-body">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-300">
                    <th className="text-left py-2 px-3 font-semibold">Serial</th>
                    <th className="text-left py-2 px-3 font-semibold">Model</th>
                    <th className="text-left py-2 px-3 font-semibold">Manufacturer</th>
                ...
                <tbody>
                  {parsedRows.slice(0, 10).map((row, idx) => (
                    <tr key={idx} className="border-b border-gray-200 hover:bg-gray-50">
                      <td className="py-2 px-3 font-mono text-xs">{row[Object.keys(columnMapping).find((k) => columnMapping[k] === 'serial_number') || ''] || '-'}</td>
                      <td className="py-2 px-3">{row[Object.keys(columnMapping).find((k) => columnMapping[k] === 'model') || ''] || '-'}</td>
                      <td className="py-2 px-3">{row[Object.keys(columnMapping).find((k) => columnMapping[k] === 'manufacturer') || ''] || '-'}</td>
                      <td className="py-2 px-3">{row[Object.keys(columnMapping).find((k) => columnMapping[k] === 'category') || ''] || '-'}</td>
                      <td className="py-2 px-3">{row[Object.keys(columnMapping).find((k) => columnMapping[k] === 'company') || ''] || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
