...
const SSO_ERROR_MESSAGES: Record<string, string> = {
  ...
  sso_provider_denied: 'Sign-in was cancelled.',
  sso_state_mismatch: 'Sign-in session expired. Please try again.',
  sso_token_exchange_failed: 'Could not complete sign-in with the provider.',
  sso_profile_failed: 'Could not load your profile from the provider.',
  sso_no_account:
    'No account found for that email. Contact an administrator if you should have access.',
  sso_no_employee:
    'Your email is in our work domain but no matching employee record was found.',
...
function LoginPageInner() {
  ...
  const [ssoConfig, setSsoConfig] = useState<{
    ...
    google: boolean;
    microsoft: boolean;
  }>({ enabled: false, google: false, microsoft: false });

  // On mount: fetch SSO availability and surface any error from the callback
  useEffect(() => {
    fetch('/api/auth/sso-enabled')
      .then((r) => r.json())
      .then((d: { enabled: boolean; providers?: { google: boolean; microsoft: boolean } }) => {
        setSsoConfig({
          enabled: !!d.enabled,
          google: !!d.providers?.google,
          microsoft: !!d.providers?.microsoft,
        });
      })
      .catch(() => {
        // Fallback: leave SSO disabled
      });

    const errCode = searchParams?.get('error');
    if (errCode && SSO_ERROR_MESSAGES[errCode]) {
      setError(SSO_ERROR_MESSAGES[errCode]);
    }
  }, [searchParams]);
            ...
            <div className="space-y-3">
              {ssoConfig.google && (
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 py-3 rounded-lg text-sm font-medium active:scale-[0.98] transition-all disabled:opacity-50"
                  style={{
              ...
              {!ssoConfig.google && !ssoConfig.microsoft && (
                <button
                  type="button"
                  onClick={() => setError('Single sign-on is not yet configured. Use email + password.')}
                  className="w-full flex items-center justify-center gap-3 py-3 rounded-lg text-sm font-medium opacity-60 cursor-not-allowed"
                  style={{
                    border: '1px dashed rgba(196, 198, 206, 0.5)',
                    color: OUTLINE,
                  }}
                >
                  Single sign-on (configure SSO_ENABLED to enable)
                </button>
              )}
            </div>
